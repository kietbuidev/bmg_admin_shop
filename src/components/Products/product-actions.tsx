...
const PAGE_ROUTE_PREFIX = "/products";
...
export function ProductActions({
  ...
  function handleUpdate() {
    setIsOpen(false);
    router.push(`${PAGE_ROUTE_PREFIX}/${productId}/edit`);
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success(`Đã xoá sản phẩm "${productName}"`);
      router.refresh();
    } catch (error) {
      console.error(error);
  ...
  return (
    <Dropdown isOpen={isOpen} setIsOpen={setIsOpen}>
      <DropdownTrigger
        ...
        aria-label={`Tùy chọn cho sản phẩm ${productName}`}
      >
